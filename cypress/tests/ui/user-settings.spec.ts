...
describe("User Settings", function () {
  ...
  it("updates first name, last name, email and phone number", function () {
    ...
    cy.get("#user-settings-firstName-input").should("be.visible").clear().type("John");
    cy.get("#user-settings-lastName-input").should("be.visible").clear().type("Doe");
    //user clicks on the first and last name text fields, clearing the current information and entering in new name data
    cy.get("#user-settings-email-input").should("be.visible").clear().type("johndoe@gmail.com");
    cy.get("#user-settings-phoneNumber-input").should("be.visible").clear().type("215-265-8957");
    //user clicks on the email and phone text fields, clearing the current information and entering in new data
    cy.getBySel("user-settings-submit").should("be.visible").click();
    //user clicks save button, saving new user settings data
  });
});
