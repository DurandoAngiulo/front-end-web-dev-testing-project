...
describe("New Transaction", function () {
  ...
  it("navigates to the new transaction form, selects a user and submits a transaction payment", function () {
    ...
    cy.getBySel("nav-top-new-transaction").should("be.visible").click();
    //user clicks on new transaction button
    cy.getBySel("user-list-item-qywYp6hS0U").should("be.visible").click();
    //user clicks on user they'd like to make their transaction with
    cy.get("#amount").should("be.visible").type("10");
    cy.get("#transaction-create-description-input").should("be.visible").type("End-to-End Testing");
    cy.getBySel("transaction-create-submit-payment").should("be.visible").click();
    //user enters their transaction amount and payment note, then clicks submit, sending payment to the other user
  });
});
